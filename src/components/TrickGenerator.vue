<template style="position: relative">
  <v-sheet class="d-flex flex-column align-center mx-2" id="trick-container" rounded color="grey lighten-2" :height="height" elvation="3" :width="width">
    <v-sheet id="belt-select" color="white" elevation="6" rounded class="d-flex flex-column mt-4 mx-4 mb-2" :width="beltSelectWidth" :height="beltSelectHeight">
      ...
      <v-select item-color="black" color="black"  background-color="white" label="From Belt Level" v-if="competitionMode === false"
                class="ma-2 mt-4" dense outlined :hide-details="true" :errors="false" :items="beltLevelArray" v-model="fromBeltLevel"></v-select>
      <div v-bind:style="{height: beltSelectBeltHeight, backgroundColor: fromBeltLevel }" v-if="competitionMode === false"></div>
      <!-- to belt level select -->
      <v-select item-color="black" color="black"  background-color="white" label="To Belt Level" v-if="competitionMode === false"
                class="ma-2 mt-4" dense outlined :hide-details="true" :errors="false" :items="beltLevelArray" v-model="toBeltLevel"></v-select>
      <div v-bind:style="{height: beltSelectBeltHeight, backgroundColor: toBeltLevel }" v-if="competitionMode === false"></div>
      <!-- from belt level select -->
      <v-select item-color="black" color="black"  background-color="white" label="Competition" v-if="competitionMode === true"
                class="ma-2 mt-4" dense outlined :hide-details="true" :errors="false" :items="competitionSelectionArray" v-model="competitionSelection"></v-select>
      <div v-bind:style="{height: beltSelectBeltHeight, backgroundColor: 'rgb(234,191,48)', boxShadow: '0 0 2px 0 ' + 'rgb(202,165,40)' }" v-if="competitionMode === true"></div>
      <!-- to belt level select -->
      <v-select item-color="black" color="black"  background-color="white" label="Bracket Selection" v-if="competitionMode === true"
                class="ma-2 mt-4" dense outlined :hide-details="true" :errors="false" :items="bracketSelectionArray" v-model="bracketSelection"></v-select>
      <div v-bind:style="{height: beltSelectBeltHeight, backgroundColor: bracketSelectionColor[bracketSelection] }"  v-if="competitionMode === true"></div>
      <!-- trick count select -->
  ...
  export default {
    ...
    data(){
      return{
        ...
        competitionSelectionArray: ['Battle At the Border 2021'],
        bracketSelectionArray: ['Beginner', 'Intermediate', 'Advanced', 'Open', 'Open (Top 8)'],
        competitionSelection: 'Battle At the Border 2021',
        bracketSelection: 'Beginner',
        bracketArrayFinal: [],
        bracketSelectionColor: {
          'Battle At the Border 2021': {
            'Beginner': 'White',
            'Intermediate': 'Orange',
            'Advanced': 'Blue',
            'Open': 'Brown',
        ...
        trickListButtonPositionTopLookUpTable:{
          'xs': 2.5,
          'sm': 2.5,
          'md': 2.5,
          'lg': 2.5,
        ...
        trickListButtonPositionRightLookUpTable:{
          'xs': 2.5,
          'sm': 2.5,
          'md': 2.5,
          'lg': 2.5,
        ...
        competitionButtonPositionTopLookUpTable:{
          'xs': 7.5,
          'sm': 7.5,
          'md': 7.5,
          'lg': 7.5,
        ...
        competitionButtonPositionRightLookUpTable:{
          'xs': 2.5,
          'sm': 2.5,
          'md': 2.5,
          'lg': 2.5,
    ...
    watch:{
        //changes the default trick count when the user switches between competition mode and the trick generator
        competitionMode() {
          if(this.competitionMode === true){
            setTimeout(() => {
              this.competitionSelection = 'Battle At the Border 2021';
              this.bracketSelection = 'Beginner';
            }, 50)
            return this.trickCount = 1
          } else {
            return this.trickCount = 3
          }
        }
    },
    computed:{
      ...
      trickListButtonPositionTop(){
        return this.trickListButtonPositionTopLookUpTable[this.$vuetify.breakpoint.name]
      },
      trickListButtonPositionRight(){
        return this.trickListButtonPositionRightLookUpTable[this.$vuetify.breakpoint.name]
      },
      competitionButtonPositionTop(){
        return this.competitionButtonPositionTopLookUpTable[this.$vuetify.breakpoint.name]
      },
      competitionButtonPositionRight(){
        return this.competitionButtonPositionRightLookUpTable[this.$vuetify.breakpoint.name]
      },
    },
    methods:{
